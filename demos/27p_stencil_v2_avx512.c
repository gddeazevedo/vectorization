...
void bc_matvec_avx512_v1(const BlockedCSR *restrict A, const double *restrict x, double *restrict y) {
    ...
    for (int brow = 0; brow < A->nb; brow++) {
        ...
        double *yrow = &y[(size_t)brow * bs]; // 3 elementos por row

        int p = row_start;
        int bcol = A->ja[p];
        const double *blk  = &A->vals[(size_t)p * bs * bs]; // 9 elementos
        const double *xcol = &x[(size_t)bcol * bs]; // 3 elementos

        __m512d vx = _mm512_set_pd(xcol[1], xcol[0], xcol[2], xcol[1], xcol[0], xcol[2], xcol[1], xcol[0]);
        __m512d vb = _mm512_loadu_pd(blk); // pode acessar memoria invalida caso row_start == row_end
        __m512d y_acc = _mm512_mul_pd(vb, vx);
        double y_acc_extra = blk[8] * xcol[2];
        ...
        for (int p = row_start + 1; p < row_end; p++) {
            ...
            vx = _mm512_set_pd(xcol[1], xcol[0], xcol[2], xcol[1], xcol[0], xcol[2], xcol[1], xcol[0]);
            vb = _mm512_loadu_pd(blk);
            y_acc = _mm512_fmadd_pd(vb, vx, y_acc);
            y_acc_extra += blk[8] * xcol[2];
        }

        yrow[0] = _mm512_mask_reduce_add_pd(MASK_SUM_FIRST_3, y_acc);
        yrow[1] = _mm512_mask_reduce_add_pd(MASK_SUM_MID_3, y_acc);
        yrow[2] = _mm512_mask_reduce_add_pd(MASK_SUM_LAST_2, y_acc) + y_acc_extra;
    }
}

void bc_matvec_avx512_v2(const BlockedCSR * restrict A, const double * restrict x, double * restrict y) {
    int bs = A->bs; // = 3
    ...
    for (int brow = 0; brow < A->nb; brow++) {
        ...
        for (int p = row_start; p < row_end; p++) {
            ...
            // Monta x replicado: [x0, x1, x2, x0, x1, x2, x0, x1]
            __m512d vx = _mm512_set_pd(
                xcol[1], xcol[0],
                xcol[2], xcol[1], xcol[0],
                xcol[2], xcol[1], xcol[0]
            );
...
void bc_matvec_avx512_v3(const BlockedCSR * restrict A, const double * restrict x, double * restrict y) {
    int bs = A->bs; // = 3 neste caso
